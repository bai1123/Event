$(function(){
    ...
    function renderPage(total){
        ...
        layui.laypage.render({
            ...
            // 触发jump回调的方式有两种：
            // 1.点页码的时候，会触发jump回调
            // 2.只要调用layui.laypage.render()方法，就会触发jump回调
            jump:function(obj,first){
                // 可以通过first的值来判断是通过哪种方式出发的jump回调
    ...
    $('tbody').on('click','.btn-delete',function(){
        ...
        layui.layer.confirm('确认删除？',{icon:3,title:'提示'},function(index){
            $.ajax({
                method:'GET',
                ulr:'/my/article/delete/'+id,
                success:function(res){
                    if(res.status!=0){
                        return layui.layer.msg('删除文章失败！')
                    }
                    layui.layer.msg('删除文章成功！')
                    // 当数据删除完成后，需要判断当前这一页中，是否还有剩余的数据。如果没有剩余的数据了，则让页码值-1之后，在重新调用initTable方法
                    if(len===1){
